...
- 依赖管理（Gemfile）
  - 根据 Ruby 版本选择 github-pages 或显式依赖 jekyll/minima/liquid/kramdown-parser-gfm/webrick/csv/base64/bigdecimal
...
### 依赖管理（Gemfile）
- 版本策略
  - 当 RUBY_VERSION >= 4.0.0 时，显式声明 jekyll、minima、liquid、kramdown-parser-gfm、webrick、csv、base64、bigdecimal，并在 jekyll_plugins 分组中声明 sitemap、seo-tag、feed
...
## 依赖分析
- 直接依赖
  - jekyll、minima、liquid、kramdown-parser-gfm、webrick、csv、base64、bigdecimal（Ruby 4.0+ 分支）
...
- 本地与线上行为不一致
  - 线上使用 GitHub Pages 指定 Ruby 版本；本地尽量保持与线上一致或使用 github-pages 聚合包
...
## 结论
通过合理的仓库命名与分支配置，结合 Gemfile 的依赖管理与自定义插件的质量保障，可以在本地高效开发并在 GitHub Pages 上稳定自动构建与发布。遵循本文的安装与排障指引，可显著降低环境问题与构建失败的概率。
...
### 依赖管理与构建命令
- 依赖管理
  - 使用 Gemfile 声明依赖；Ruby 4.0+ 分支采用显式依赖，其他分支使用 github-pages 聚合包
- 常用命令
