...
## 更新摘要
**所做更改**   
- 更新了 .qoderignore 配置中 repowiki 排除模式的描述，从目录路径匹配改为文件级匹配
- 增强了索引控制粒度的说明，反映更精确的排除策略优化
- 更新了相关章节以体现新的排除模式带来的改进效果
...
- Qoder 忽略配置（.qoderignore）
  ...
  - **更新**：当前规则使用 `.qoder/repowiki/*` 文件级匹配模式，仅排除 repowiki 目录内的所有文件，提供更好的索引控制粒度。
- 站点配置（_config.yml）
...
- **更新**：规则说明已优化，采用更精确的文件级匹配模式：
  - 注释行以 # 开头，用于说明规则用途。
  - 支持类似 .gitignore 的通配符与路径匹配。
  - **当前规则**：使用 `.qoder/repowiki/*` 文件级匹配模式，仅排除 repowiki 目录内的所有文件，而非整个目录。这种策略提供了更好的索引控制粒度，允许保留目录结构的同时排除具体文件内容。
- 适用场景：
  - 大型文档/知识库目录
  - 临时生成的中间产物
  - 第三方库或缓存目录
...
## 性能与构建特性
- 索引性能
  - **更新**：通过 .qoderignore 使用 `.qoder/repowiki/*` 文件级匹配模式，可以更精细地控制索引范围，减少索引时间与内存占用，同时保留目录结构的完整性。
- 构建稳定性
...
- Qoder 索引异常
  ...
  - **更新**：确认 `.qoder/repowiki/*` 文件级匹配模式是否正确排除了 repowiki 目录内的文件，同时保留了目录结构
- 构建产物未更新
...
- **.qoderignore 是控制 Qoder 索引范围的关键配置，能有效提升索引效率与结果质量**
  - **更新**：采用 `.qoder/repowiki/*` 文件级匹配模式提供了更精确的索引控制粒度，在排除具体文件内容的同时保留目录结构
...
- **最佳实践**：优先使用文件级匹配模式（如 `*/\*`）而非目录级匹配，以获得更灵活的索引控制能力