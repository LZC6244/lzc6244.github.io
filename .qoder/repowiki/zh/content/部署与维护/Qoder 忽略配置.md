...
## 更新摘要
**所做更改**   
- 更新了 .qoderignore 配置中 repowiki 排除模式的描述，从目录级匹配 (.qoder/repowiki/) 更新为递归通配符模式 (.qoder/repowiki/**)
- 增强了索引控制粒度的说明，反映更精确的文件匹配能力
- 更新了相关章节以体现新的递归通配符模式带来的性能改进效果
...
- Qoder 忽略配置（.qoderignore）
  ...
  - **更新**：当前规则使用 `.qoder/repowiki/**` 递归通配符模式，完全排除 repowiki 目录及其所有子内容和文件，提供更精确的文件匹配能力和更高的索引效率。
- 站点配置（_config.yml）
...
- **更新**：规则说明已优化，采用更高效的递归通配符匹配模式：
  - 注释行以 # 开头，用于说明规则用途。
  - 支持类似 .gitignore 的通配符与路径匹配。
  - **当前规则**：使用 `.qoder/repowiki/**` 递归通配符模式，完全排除 repowiki 目录及其所有子内容和文件。这种策略提供了更精确的文件匹配能力，通过递归匹配确保覆盖整个生成文档目录的所有层级结构，显著减少索引时间和内存占用。
- 适用场景：
  - 大型文档/知识库目录
  - 临时生成的中间产物
  - 第三方库或缓存目录
...
Match --> |是| Skip["跳过整个目录及所有子内容<br/>使用递归通配符匹配"]
Match --> |否| Index["加入索引"]
Skip --> Next["继续扫描下一个路径"]
Index --> Next
Next --> End(["完成索引"])
...
## 性能与构建特性
- 索引性能
  - **更新**：通过 .qoderignore 使用 `.qoder/repowiki/**` 递归通配符模式，可以更全面地排除整个 repowiki 目录及其所有内容，显著减少索引时间与内存占用，提升整体索引效率。递归通配符模式确保了无论 repowiki 目录下有多少层子目录结构，都能被完全匹配和排除。
- 构建稳定性
...
- Qoder 索引异常
  ...
  - **更新**：确认 `.qoder/repowiki/**` 递归通配符模式是否正确排除了整个 repowiki 目录及其所有子内容，确保深层嵌套的文件也能被正确忽略
- 构建产物未更新
...
- **.qoderignore 是控制 Qoder 索引范围的关键配置，能有效提升索引效率与结果质量**
  - **更新**：采用 `.qoder/repowiki/**` 递归通配符模式提供了更精确的文件匹配能力，通过递归匹配确保完全覆盖整个生成文档目录的所有层级结构，优化索引性能
...
- **最佳实践**：对于大型生成目录，优先使用递归通配符模式（如 `directory/**`）而非简单的目录级匹配，以获得更好的性能优化效果和更精确的匹配能力